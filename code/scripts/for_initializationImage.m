...
for folderNum = 5
    ...
    for meas = 1:numIter
        ...
        if contains(fileName, 'RASERHybrid_dev1')
            ...
            initMaps = ...
                createInitMpasForT2star_m3(kSpaceEPI(:,:,:,:,:,1:end,1), trajEPI(:,:,:,:,:,1:end,1), TEsEPI, R2, synthesizedRAREimages, binaryMaskRARE, protPara);

            
            [M0star, R2star, T2star, sensEPI, synthesizedEPIimages, B0] = ...
                T2starReco_n(kSpaceEPI(:,:,:,:,:,1:end,1), trajEPI(:,:,:,:,:,1:end,1), TEsEPI(:,:,:,:,:,1:end), initMaps, protPara);
