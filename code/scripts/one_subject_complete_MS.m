...
baseFolderName = 'MDC_0216_MOBA_MS_';
numFolders = 8; % Number of folders to process
...
for folderNum = 1:numFolders
    ...
    for meas = 1:numIter
        ...
        if contains(fileName, 'RASERHybrid_dev1')
            ...
            [kSpaceRARE_, kSpaceEPI_,  trajRARE_, trajEPI_, TEsRARE, TEsEPI, protPara, config] = ...
                preProcessRawData(folderWitRawData, fileName,isOversamplingRemoved, applyNoiseDecorrelation, nVirtualCoils);
            New_order = [1 5 2 6 3 7 4];
            kSpaceRARE = kSpaceRARE_(:,:,:,:,:,:,New_order);
            kSpaceEPI = kSpaceEPI_(:,:,:,:,:,:,New_order);
            trajRARE = trajRARE_(:,:,:,:,:,:,New_order);
            trajEPI = trajEPI_(:,:,:,:,:,:,New_order);
            % T2 mapping
            ...
            protPara.T2StarMobaPara.scalingFactorR2star = 2.1;
