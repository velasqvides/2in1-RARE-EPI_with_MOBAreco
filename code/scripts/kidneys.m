...
baseFolderName = 'kidneys_240125_';
...
for folderNum = 1:numFolders
    ...
    for meas = 1:numIter
        ...
        if contains(fileName, 'RASERHybrid_dev1')
            ...
            image = ReconstructImageGridding(kSpaceRARE(:,:,:,:,:,1:end,:), trajRARE(:,:,:,:,:,1:end,:));
            imagesRARE = ReconstructImagePics(kSpaceRARE(:,:,:,:,:,1:end,:), trajRARE(:,:,:,:,:,1:end,:));
            imagesEPI = ReconstructImagePics(kSpaceEPI(:,:,:,:,:,1:end,:), trajEPI(:,:,:,:,:,1:end,:));
            
            iter = 5;
            images_RARE_abs = abs(imagesRARE); % in case images are reconstructed with phase perservation methods like pics
            images_RARE_abs_norm = normalizeArray(images_RARE_abs);
            images_RARE_abs_norm = squeeze(images_RARE_abs);
            TEsMSE = (squeeze(TEsRARE))';
            for i=1:5
            [T2RARE(:,:,i), M0MSE(:,:,i)] = T2analysis_withM0(images_RARE_abs_norm(:,:,2:end,i), TEsMSE(1,2:end).*1000);
            end
            visualizeMaps_newColorMaps(fliplr(rot90(T2RARE(:,:,4))),175,'T2');
            visualizeMaps(rot90(T2RARE),150);

            imagesEPI = ReconstructImagePics(kSpaceEPI(:,:,:,:,:,1:end,:), trajEPI(:,:,:,:,:,1:end,1));

            iter = 5;
            images_EPI_abs = abs(imagesEPI); % in case images are reconstructed with phase perservation methods like pics
            images_EPI_abs_norm = normalizeArray(images_EPI_abs);
            images_EPI_abs_norm = squeeze(images_EPI_abs);
            TEsMGRE = (squeeze(TEsEPI))';
for i=1:5
            [T2EPI(:,:,i), M0EPI(:,:,i)] = T2analysis_withM0(images_EPI_abs_norm(:,:,1:end,i), TEsMGRE(1,1:end));
end
[M0MGRE, T2starMGRE, R2starMGRE, ~] = performMobafitEPI(images_EPI_abs_norm(:,:,:,:,:,1:end,1), TEsEPI(:,:,:,:,:,1:end), iter);
            visualizeMaps_newColorMaps(fliplr(rot90(T2EPI(:,:,4))),80,'T1');
            visualizeMaps(rot90(T2MSE),150);
            % T2 mapping
            protPara.T2MobaPara.mobaOversampling = 2;
            ...
            protPara.T2MobaPara.regMoba = 0.0001;

            [M0, R2, T2, binaryMaskRARE, sensRARE, synthesizedRAREimages] = ...
                T2Reco(kSpaceRARE(:,:,1:4:135,:,:,2:end,4), trajRARE(:,:,1:4:135,:,:,2:end,4), TEsRARE(:,:,:,:,:,2:end), protPara);
            % T2_ = prepareMaps(T2, binaryMaskRARE, protPara);
            % visualizeMaps(T2_,150);
            % visualizeMaps_newColorMaps(fliplr(rot90(T2)),175,'T2');
            ...
            protPara.T2StarMobaPara.mobaOversampling = 2;
            ...
            protPara.T2StarMobaPara.B0smoothLevel = 220;
            protPara.T2StarMobaPara.B0scaling = 1;
            ...
            initMaps = ...
                createInitMpasForT2star_m3(kSpaceEPI(:,:,1:4:135,:,:,1:end,4), trajEPI(:,:,1:4:135,:,:,1:end,4), TEsEPI, R2, synthesizedRAREimages, binaryMaskRARE, protPara);

            % trajEPI_ = bart(sprintf('scale 0.5'),  trajEPI);
            [M0star, R2star, T2star, sensEPI, synthesizedEPIimages, B0] = ...
                T2starReco_n(kSpaceEPI(:,:,1:4:135,:,:,1:end,4), trajEPI(:,:,1:4:135,:,:,1:end,4), TEsEPI(:,:,:,:,:,1:end), initMaps, protPara);
            % T2star_ = prepareMaps(T2star, binaryMaskRARE, protPara);

            % visualizeMaps_newColorMaps(fliplr(rot90(T2star)),80,'T1');
            % end
