...
R2prime = R2star - R2;
R2MSE = readcfl('R2');
R2MGRE = readcfl('R2star');
R2primeRef = R2MGRE -R2MSE;

R2prime = readcfl('M0');
R2primeRef = readcfl('M0');
R2prime = abs(R2prime);
R2primeRef = abs(R2primeRef);
R2prime = normalizeArray(R2prime);
R2primeRef = normalizeArray(R2primeRef);

% figName  = 'R2';
% finalDir = 'R2';
% maxValue = 20;
% saveRelaxationMaps(10 .* R2, maxValue, figName, finalDir, config);
config.dirToSave='/home/Velasquez/Documents/PhD_project/projects/RestOfPlots_test_retest_figure';
...
ROIsize = 7;
    currentROIs = ROIs_testRetest_002;
    nSlices = 27;
for i=1:nSlices
        startIdx = (i - 1) * 10 + 1;
        endIdx = i * 10;
    ...
    plotScatter(means_T2starAll, means_T2starMGREall, config, stringName, 1, 0.5, 2.5, saveOutput);
    [mean_diff_T2star, std_diff_T2star] = plotBlandAltman(means_T2starAll, means_T2starMGREall, config, stringName, 1, 0.5, 2.5, saveOutput);
