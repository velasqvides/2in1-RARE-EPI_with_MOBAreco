...
for folderNum = 1:numFolders
    ...
    for meas = 1:numIter
        ...
        if contains(fileName, 'RASERHybrid_dev1')
            ...
            protPara.T2StarMobaPara.nIterMoba = 10;
            ...
            initMaps = ...
                createInitMpasForT2star_m3(kSpaceEPI(:,:,:,:,:,1:end,4), trajEPI(:,:,:,:,:,1:end,4), TEsEPI, R2, synthesizedRAREimages, binaryMaskRARE, protPara);

            % trajEPI_ = bart(sprintf('scale 0.5'),  trajEPI);
            [M0star, R2star, T2star, sensEPI, synthesizedEPIimages, B0] = ...
                T2starReco_n(kSpaceEPI(:,:,:,:,:,1:end,4), trajEPI(:,:,:,:,:,1:end,4), TEsEPI(:,:,:,:,:,1:end), initMaps, protPara);
            % T2star_ = prepareMaps(T2star, binaryMaskRARE, protPara);

            % visualizeMaps_newColorMaps(rot90(T2star),125,'T1');
            % end
