function [T2starOut, B0out] = T2starReco_likePaper(kSpace, traj, TEs, protPara, binaryMaskOut, config,specialStamp)
...
R_M1_init_Fout =zeros(recoSize,recoSize,1,1,1,1,5,nSlices);
...
%     initR2star = initR2star.*0.01.*1.6;
    ...
    initF = bart('extract 6 1 2',R_m0_1fe);
%     toPad = (recoSize - baseRes)/2;
%     initW = padarray(initW,[toPad toPad]);
initR2starW =zeros(size(initW));
initR2starF =zeros(size(initW));
    R_M1_init_F = bart('join 6',initW, initR2starW,initF, initR2starF, initB0);
...
nSlices=1;
for slice = 1:nSlices
    writecfl('R_M1_init_F_file',R_M1_init_Fout(:,:,:,:,:,:,:,slice));
    % moba reconstruction: multi-echo R2* mapping
    [recoEPI, sensEPI] = ...
        bart(...
        sprintf...
        ('moba -G -m2 -rQ:0.1 -rS:0 -rW:3:64:0.1 -rT:3:64:0.1 -u0.01 -o%4.2f  -i10 -C100 -R3 -k --kfilter-2 -I R_M1_init_F_file	 -d4 -t',mobaOversampling),...
