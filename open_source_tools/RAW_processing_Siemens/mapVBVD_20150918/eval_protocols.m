function tab = eval_protocols(pathstr)
    ...
    for k=1:numel(filelist)
        ...
        if iscell(prot)
            prot = prot{size(prot,1)};
        end
        file{k} = filelist(k).name;
        if isfield(prot.Meas,'SequenceDescription')
            seq{k} = prot.Meas.SequenceDescription;
        end
        if isfield(prot.Config,'SequenceDescription')
            seq{k} = prot.Config.SequenceDescription;
        end
        ...
        if isfield(prot.Meas,'NSetMeas')
            NSetMeas = prot.Meas.NSetMeas;
            NRepMeas = prot.Meas.NRepMeas;
            NImageCols = prot.Meas.NImageCols;
            NImageLins = prot.Meas.NImageLins;
            NSlc = prot.Meas.NSlc;
        end
        if isfield(prot.Config,'NSetMeas')
            NSetMeas = prot.Config.NSetMeas;
            NRepMeas = prot.Config.NRepMeas;
            NImageCols = prot.Config.NImageCols;
            NImageLins = prot.Config.NImageLins;
            NSlc = prot.Config.NSlc;
        end
        TRvol{k} = TR{k} * round(1e6 *prot.MeasYaps.lScanTimeSec/prot.MeasYaps.alTR{1}/NSetMeas)/1000;
        rep{k} = NSetMeas * NRepMeas;
    ...
    clear NSetMeas NRepMeas NSlc NImageCols NImageLins;
    seq = seq.';
    mid = mid.';
