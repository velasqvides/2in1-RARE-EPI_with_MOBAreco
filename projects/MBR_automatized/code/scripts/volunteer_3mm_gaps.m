folderWitRawData = 'volunteer_gaps3mm_noRRIshiming_301224';
...
for meas = 1:numIter
    ...
    % order = generateReorderingSequence(protPara.nSlices);
    % kSpaceRARE = kSpaceRARE(:,:,:,:,:,:,order);
    % kSpaceEPI = kSpaceEPI(:,:,:,:,:,:,order);
    % trajRARE = trajRARE(:,:,:,:,:,:,order);
    % trajEPI = trajEPI(:,:,:,:,:,:,order);
    ...
    T2(T2 < 0) = 2000; %set negative values in CSF to TR=2000 ms for beter visualization
    ...
    T2star(T2star < 0) = 2000; %set negative values in CSF to TR=2000 ms for beter visualization
