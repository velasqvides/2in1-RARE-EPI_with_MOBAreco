function [T2starOut] = T2starReco_initWmoba(kSpace, traj, TEs, protPara, lastRAREimageOut, R2out, binaryMaskOut, config)
...
mobaOversampling = 1;
recoSize = baseRes*2*mobaOversampling; % not sure yet why the factor of 2
...
for slice = 1:1
    ...
    writecfl('init_file',R_M1_init_Fout(:,:,:,:,:,:,:,slice));
    [recoEPI, sensEPI] = ...
        bart(...
        sprintf...
        ('moba -G -m3 -rQ:1 -rS:0 -rW:3:64:1 -rT:3:64:0.1 -u0.01 -o%4.2f -i10 -C100 -R3 -k --kfilter-2 -d4 -I init_file -t',mobaOversampling),...
