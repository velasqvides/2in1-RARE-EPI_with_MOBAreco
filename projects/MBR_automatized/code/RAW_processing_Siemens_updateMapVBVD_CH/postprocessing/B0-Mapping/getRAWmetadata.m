...
% PULSE PARAMETERS
Pulse.Nucleus=MrProt.TXSPEC.NucleusInfo(1,1).Nucleus;

if strcmp(Pulse.Nucleus,'1H')
    Pulse.gamma_Nucleus_rad_s_T = 2.67522212E8; %[rad/s/T]
elseif strcmp(Pulse.Nucleus,'23Na')
    Pulse.gamma_Nucleus_rad_s_T = 7.09051647E7;%[rad/T/s]
else
    disp('unknown nucleus used! no gamma available')
    return
end
...
Pulse.SEG=MrProt.WiPMemBlock.alFree(2); %segments (samples)of the B1 pulse [1]
Pulse.TotalDuration=MrProt.WiPMemBlock.alFree(3); %duration of the B1 pulse [us]

Pulse.OLD=MrProt.WiPMemBlock.adFree(1); % single loop duration [us]
Pulse.HL=MrProt.WiPMemBlock.adFree(2); %half loops of the B1 pulse [1]
...
Pulse.NominalB1=1E6*(Pulse.LoopFA*pi/180)/(Pulse.TotalDuration*1E-6*Pulse.gamma_Nucleus_rad_s_T); %[uT]
