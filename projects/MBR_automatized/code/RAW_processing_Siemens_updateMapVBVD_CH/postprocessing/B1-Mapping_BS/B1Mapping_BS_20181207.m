function [Maps,detailedB1postprocessingData]=B1Mapping_BS_20181207(combinedImage,MrProt,newDataDims,config)
...
if strcmp(Sequence.Nucleus,'1H')                                                 % Strcmp to avoid error
    Pulse.gamma_Nucleus_rad_s_T = 2.67522212E8; %[rad/s/T]
elseif strcmp(Sequence.Nucleus,'23Na')                                               
    Pulse.gamma_Nucleus_rad_s_T = 0.70761E8; %[rad/s/T]
else
    disp('ERROR: no nucleus information found!!!')
    return
end
...
SimulatedSensitivity=Sensitivity_build_mex(Pulse,Sequence,SimulatedSensitivity);
...
% sometimes due to wrong unwrapping the sensitivity curve starts at phase accrual of 360? and not at 0?.Then you have to subtract 360?!
Info.subtract360fromPhaseAccrual=input('Are you happy with the simulated Phase Accrual [Enter] or do you want to substract 360? from it [1]? ');

while  (~isempty(Info.subtract360fromPhaseAccrual))
...
2016-09-21 -now subfunctions also support sodium imaging:
            ...
            (Olli Weinberger)
2016-10-12 - optional input option for subtraction of 360? if sensitivity 
            curve does not cover a phase accrual starting from 0? but from 360?
            (Olli Weinberger)
2018-10-05 - Puls and Sequence Info updated
