classdef imtool3D < handle
    ...
    methods
        ...
        function tool = imtool3D(varargin)  %Constructor
            ...
            % AK: keypress events
            set(fig,'KeyPressFcn',@tool.KeyPressFunction);
            ...
            %Create floodfill tool button
            tool.handles.Tools.FloodFill        =   uicontrol(tool.handles.Panels.ROItools,'Style','pushbutton','String','','Position',[buff buff+7*w w w],'TooltipString','Flood Fill');
            icon_profile = makeToolbarIconFromPNG([iptdir '/Floodfill_16.png']);
            set(tool.handles.Tools.FloodFill ,'Cdata',icon_profile)
            fun=@(hObject,evnt) FloodFillImageCallback(hObject,evnt,tool);
            set(tool.handles.Tools.FloodFill ,'Callback',fun)
        ...
        function KeyPressFunction(tool,src,event)
            
%             fprintf('Keypress detected: %s.\n', event.Character);
            
            switch event.Character
                case 'q'
                    currMask = tool.getCurrentMaskSlice;
                    ker = strel('disk',2);
                    currMask = imerode(currMask,ker);
                    tool.setCurrentMaskSlice(currMask);
                    
                case 'e'
                    currMask = tool.getCurrentMaskSlice;
                    ker = strel('disk',2);
                    currMask = imdilate(currMask,ker);
                    tool.setCurrentMaskSlice(currMask);
                    
                case 'w'
                    currMask = tool.getCurrentMaskSlice;
                    tool.setCurrentMaskSlice(circshift(currMask,-1,1));
                ...
                case 'a'
                    currMask = tool.getCurrentMaskSlice;
                    tool.setCurrentMaskSlice(circshift(currMask,-1,2));
                ...
                case 'r'
                    currSliceNum = tool.getCurrentSlice;
                    fullMask = tool.getMask;
                    
                    if currSliceNum < size(fullMask,3)
                        prevSliceMask = fullMask(:,:,currSliceNum+1);
                        if any(prevSliceMask(:))
                            tool.setCurrentMaskSlice(prevSliceMask);
                        end
                    end
...
% AK
function FloodFillImageCallback(hObject,evnt,tool)

    pickedPoint = ginput(1);
