function [B1Map_rel,B1Map_uT,B0Map,detailedData]=B1Mapping(combinedImage,newDataDims,MrProt,config)
...
if (~isempty(Info.enUnwrapping))
    for cPar=1:nPar
        ...
        title(['Phase difference used for B_0-Map. slice: ', num2str(cPar)])
        axis image
        subplot(1,2,2)
...
close all
% cleanup workspace
clearvars -except Info Masks Magnitude1 Pulse Sequence nPar imPhaDiffB0 imPhaDiffB1 %clean up workspace
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
B1.Map_abs = B1.Map_rel.*Pulse.NominalB1./Sequence.ReqVolt.*sqrt(50).*sqrt(1000);%unit: uT/sqrt(kW)
for cPar=1:nPar
...
Info.MaxB1_uT=max(B1.Map_abs(:));
for cPar=1:nPar
    Image_plot3D(5,nPar,cPar,0,Info.MaxB1_uT,'hot',B1.Map_abs(:,:,cPar),1,1)
end
...
%}
Statistics.B1Data       =B1.Map_abs(Masks.Effective);
...
title(['AVG = ', num2str(Statistics.B1_avg), ' uT , STD = ' num2str(Statistics.B1_std),' uT'])
...
if (Info.enROI)
    Statistics.ROI.B1Data       =B1.Map_rel(Masks.Effective_ROI); 
    Statistics.ROI.relB1Max        =max(Statistics.ROI.B1Data);
    Statistics.ROI.Bins         =0 : Statistics.ROI.relB1Max/(300-1) : Statistics.ROI.relB1Max;% 1x300 vector
    ...
    set(gca,'XLim',[0 Statistics.ROI.relB1Max])
    xlabel('B_1^+ distribution inside ROI')
    ylabel('Normalized Distribution')
    title(['AVG = ', num2str(Statistics.ROI.B1_avg),' STD = ' num2str(Statistics.ROI.B1_std)])
    Image_save(fullfile(Info.Path,'B1hist_ROI'),'-dtiff')
    
    string1=sprintf('(B1act/B1nom)_ROI = %1.2f +- %1.2f',Statistics.ROI.B1_avg,Statistics.ROI.B1_std);
    %string2=sprintf('\nOld Reference Voltage: %3.1f V ',Sequence.RefVolt);
    string3=sprintf('\nAdjust Reference Voltage to %3.1f V ',Sequence.RefVolt/Statistics.ROI.B1_avg);
    disp([string1,string3])
    
end

%% cleanup workspace, prepare export and save data
clearvars -except B0 B1 Info Magnitude1 Masks Pulse Sequence SimulatedSensitivity Statistics %clean up workspace

B1Map_rel=B1.Map_rel;
B0Map=B0.Map;
B1Map_uT=B1.Map_abs;
detailedData=struct('Info',Info,'Pulse',Pulse,'Sequence',Sequence,...
    'SimulatedSensitity',SimulatedSensitivity,'Magnitude_Echo1',Magnitude1,...
    'Masks',Masks,'B0',B0,'B1',B1,'Statistics',Statistics);

save(fullfile(Info.Path,['B0B1Maps_',Info.MID]),'B0Map','B1Map_rel','B1Map_uT','-v7.3')
