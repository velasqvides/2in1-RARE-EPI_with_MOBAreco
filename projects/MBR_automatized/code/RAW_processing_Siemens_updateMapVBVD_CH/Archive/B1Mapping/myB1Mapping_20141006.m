function [B1Map_rel,B1Map_uT,B0Map_Hz,detailedB1postprocessingData]=myB1Mapping_20141006(combinedImage,MrProt,newDataDims,config)
...
%
% OUTPUT: 
% B1Map_rel = relative B1 map (wherever B1Map_rel=100% => actuaal FA = nominal FA of protocol)
...
for cPar=1:nPar
    B1.Map_rel(:,:,cPar)=Masks.MagThres(:,:,cPar).*Interpolation_B1B0(B0.Map(:,:,cPar),imPhaDiffB1(:,:,cPar)*180/pi .*Masks.MagThres(:,:,cPar),SimulatedSensitivity.DeltaPhi,SimulatedSensitivity.B0vec,SimulatedSensitivity.B1vec);
end
...
for cPar=1:nPar
    Image_plot3D(4,nPar,cPar,0,3,'hot',B1.Map_rel(:,:,cPar),1,1)
end
...
if Info.SavingString
    save(fullfile(Info.ProcessedPath,['B0B1Maps_',Info.MID]),'B0Map_Hz','B1Map_rel','B1Map_uT','-v7.3')
    disp('########################################')
