function [sensitivity,image] = combineSVD2D_selectedChannels(data,selectedChannels)
...
% selectedChannels is a vector of scalar, which specify which channels to use. 
% unused channels wont contribute to the final image.
% 
% output:
...
% modified by Olli Weinberger 06.09.2016


noSelectedChannels=length(selectedChannels);

siz = size(data);
if ndims(data)==3
    siz(4)=1;
end 
if sum(siz(3)<selectedChannels)
    disp('selected channel does not exist')
    return
end
data = data(:,:,selectedChannels,:);
...
sensitivity = reshape(sensitivity, [siz([1 2]),noSelectedChannels]);
